...
/**
 * Donut Model
 * Generates markup and behavior for an arc
 *
 * Usage:
 * var donut = new r3.Donut(opts);
 */
var r3 = r3 || {};

r3.donut = (function(window, d3, undefined) {

  var defaults = {
    bindTo: 'body',     // if not specified, it will append to the body
    className: 'donut',
    size: {
      width: 200,
      height: 200
    ...
    startAngle: 0,      // starting angle, in degrees. can be negative
    endAngle: 360,      // ending angle, in degrees. can be negative
    thickness: null,      // the thickness of the arc
    offset: 0,          // padding between each arc section, essentially uses stroke to simulate offset, which you can style to match your background color
    sort: null,
    maxValue: null,     // useful if you want your chart to display with a background arc, like a progress bar
    background: false,  // if true, you'll have a background from your startAngle to endAngle
    colors: d3.scale.category20c(),
    accessor: function(d, i) {
      return d;   // accessor is used to map your data object array. use it to tell the donut which value to use
    }
  };

  var donut = {};

  donut.generate = function(config) {
  ...
  var Donut = function(config) {
    ...
    this.arc = d3.svg.arc()
      .innerRadius(this.config.radius - (this.config.thickness ? this.config.thickness - (this.config.offset / 4) : this.config.radius) )
  ...
  function removeArcTween(a, i, donut) {
    var emptyArc = {
      ...
      interpolate = d3.interpolate(a, emptyArc);
    return function(t) {
      return donut.arc(interpolate(t));
    };
  }
